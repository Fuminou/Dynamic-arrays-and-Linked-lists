...
struct dynarray* dynarray_create() {
  struct dynarray* s = (struct dynarray*)malloc(sizeof(struct dynarray));
  s->capacity=2;
  s->size=0;
...
void dynarray_remove(struct dynarray* da, int idx) {  
  int n=da->size;
  //^finding the number of elements in array
  //below is removing the element and adjusting the rest of the array
...
void* dynarray_get(struct dynarray* da, int idx) {
  ...
  int n=da->size;
  if(idx>n){
    printf("idx is larger than number of elements");
  }
  else{
    return da->data[idx];  
  }
